...
- **Tools** — `retrieve_context` (pooled multi-doc hybrid retrieval via `retrieval.agentic_retrieve_context_multi_with_evidence`, which also returns the citable units the tool records into `retrieved_units` for the UI's citation chips), `reason_over_graph` (GraphRAG: explicit relation chains for multi-hop "how is X connected to Y" questions), `read_image` (VLM analysis of figures/charts embedded in the doc), `preview_edit` + `apply_edit` (edit the document — `apply_edit` is a HARD confirmation gate: it calls LangGraph `interrupt()` to pause the graph before writing, surfaces the diff, and only persists after the user resumes with "approve"), and `create_markdown_file` (export/save document-grounded content as a downloadable `.md`)
- **Conversation memory** — durable `AsyncSqliteSaver` checkpointer at `~/.laidocs/data/checkpoints.db`, keyed per global `thread_id = "session-{session_id}"` (sessions are global; doc scope is transient per request); survives backend restarts
- **Conversation compaction** — `backend/services/compactor.py` keeps token usage bounded: `compact_checkpointer_if_needed` (called *after* each stream in `chat.py`'s `finally` block) compacts the LangGraph **checkpoint** for the thread in-place — if the stored message list exceeds a token threshold it replaces everything except the last `CHECKPOINT_TAIL_PAIRS` Human/AI exchanges with a single LLM-generated summary `AIMessage`, then writes the smaller list back via the checkpointer's `aget_tuple`/`aput`. Best-effort and backend-agnostic; never touches `chat_messages` display history
...
- `POST /api/chat/stream` — SSE stream with `session_id` support (also emits `[EVIDENCE]` / `[CHAIN]` / `[EDITED]` / `[INTERRUPT] {json}` sentinels before `[DONE]`). `[INTERRUPT]` means the turn paused at the `apply_edit` confirmation gate; its payload is the edit diff (`{type, file, action, old_string, new_string}`).
- `POST /api/chat/resume` — resume a turn paused at the `apply_edit` gate: body `{doc_ids, decision: "approve"|"reject", session_id?}`. Feeds `Command(resume=decision)` on the same `thread_id`; `apply_edit` re-runs (applies on approve, leaves the file untouched on reject) and the agent finishes its turn over the same SSE trailer. Shares the `_run_turn` streamer with `/stream`.
...
- SSE streaming for chat (`POST /api/chat/stream`) and upload progress stages. `streamChat` takes a handlers object `{ onChunk, onEdited, onEvidence, onChain, onInterrupt }`; the stream's `[EVIDENCE] {json}` / `[CHAIN] {json}` sentinels carry citation + reasoning-chain payloads, and `[INTERRUPT] {json}` carries an edit-confirmation diff. On `onInterrupt`, `ChatPanel` shows an approve/reject card and calls `resumeChat(docIds, decision, handlers, sessionId)` (→ `POST /api/chat/resume`), which streams the resumed turn back into the same assistant bubble.
...
- **Document editing via chat**: `apply_edit` does NOT write immediately — it calls LangGraph `interrupt()` (the durable `AsyncSqliteSaver` checkpointer holds the suspended state) to pause the graph and hand the diff to the UI as `[INTERRUPT]`. On `/api/chat/resume` with `Command(resume="approve")`, the tool node re-executes (its read-only locate logic re-runs idempotently; the post-`interrupt()` write runs exactly once), writes through `document_store.persist_document_content` (keeps the `.md` file, SQLite `content`, and tree index in sync), sets the per-request `edited` flag, and the stream emits `data: [EDITED]` so the frontend reloads. On "reject" the file is left untouched. `_run_turn` skips checkpoint compaction while a turn is interrupted (never compact a thread with a pending task). Stale-gate guards (a pending interrupt wedges the thread — LangGraph keeps it pending even if you feed a new input): `/stream` calls `_clear_pending_interrupt` (drains a silent `Command(resume="reject")`, discarding the abandoned edit) before running a new question, and `/resume` no-ops via `_has_pending_interrupt` when nothing is suspended (guards double-click / already-cleared resumes). So a turn left interrupted across an app restart is safely auto-discarded on the user's next message rather than blocking the session. Covered by `tests/test_agent_edit_interrupt.py`.
